...
class Cti_Configurator_Helper_Components_AttributeSets extends Cti_Configurator_Helper_Components_Abstract
{
    ...
    protected function _processFile($globalFile,$localFile = null) 
    {
        if (!file_exists($globalFile)) {
            throw new Mage_Core_Exception(
                "Cannot find global configuration YAML file."
            );
        }
        ...
        if (isset($data['attribute_sets'])) {
            foreach ($data['attribute_sets'] as $set) {
                ...
                if (!$attributeSet) {
                    ...
                    $this->_inheritAttributeSet(
                        $attributeSet,
                        $inheritAttributeSetName
                    );

                    $this->log(
                        $this->__(
                            'Inherited new attributes from "%s"',
                            $inheritAttributeSetName)
                    );
                }
                ...
                foreach ($set['groups'] as $group) {
                    if (!isset($group['name'])) {
                        throw new Exception(
                            $this->__(
                                'No group name set for attribute "%s"',
                                $set['name'])
                        );
                    }

                    // Get the attribute Group ID
                    $attributeGroupId = $this->_getAttributeGroupId(
                        $attributeSet, $group['name']
                    );

                    if (!$attributeGroupId) {
                        $this->log(
                            $this->__(
                                'No attribute group found named "%s"',
                                $group['name']
                            )
                        );
                    }
                    ...
                    foreach ($group['attributes'] as $attributeCode) {
                        ...
                        $this->_assignAttributeToAttributeSet(
                            $attribute,
                            $attributeSet,
                            $attributeGroupId
                        );
                        ...
                        $this->log(
                            $this->__(
                                'Added "%s" attribute to "%s" attribute within "%s" group',
                                $attribute->getName(),
                                $attributeSet->getAttributeSetName(),
                                $group['name']
                            )
     ...
     */
    private function _inheritAttributeSet(
     ...
     * @return mixed
     */
    private function _getAttributeGroupId(
        Mage_Eav_Model_Entity_Attribute_Set $attributeSet,
        $attributeGroupName
    ) {
     ...
     */
    protected function _assignAttributeToAttributeSet(
        $attribute,
        $attributeSet,
        $attributeGroupId
    ) {
