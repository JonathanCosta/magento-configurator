...
class Cti_Configurator_Helper_Components_Config extends Cti_Configurator_Helper_Components_Abstract
{
    ...
    protected function _processComponent($data) 
    {
        ...
        if (isset($data['websites'])) {
            foreach ($data['websites'] as $websiteCode=>$configData) {
                ...
                $websiteConfig = $this->_mergeInheritables(
                    $configData,
                    $inheritables
                );
        ...
        if (isset($config['core_config'])) {
            foreach ($config['core_config'] as $config) {
                ...
                if (isset($config['encrypted']) && $config['encrypted'] == 1) {
                    $config['value'] = Mage::helper('core')->encrypt(
                        $config['value']
                    );
                }
    ...
    private function _saveWebsiteConfigItems(
        array $configs,
    ...
    private function _saveStoreConfigItems(
        array $configs,
        ...
        if (isset($config['inherit'])) {
            foreach ($config['inherit'] as $key) {
                $data = array_merge_recursive(
                    $data, $inheritables[$key]['core_config']
                );
            }
        }
        return $data;
    ...
    private function _checkAndSaveConfig(
        $path,
        $value,
        $scope = 'default',
        $scopeId = 0,
        ...
        switch ($scope) {
        case 'websites':
            $valueCheck = (string) Mage::app()->getWebsite($scopeId)
                ->getConfig($path);
            break;
        case 'stores':
        ...
        if ($value != $valueCheck) {
            ...
            case 'websites':
                $this->log($this->__(
                    'Saved path for website: %s - %s to %s',
                    $code,
                    $path,
                    $value)
                );
